...
from tools.Config import Cancel_url, amountlimit, premiumdict, Trade_url, Queryorder_url, Query_tradeprice_url, \
    pricelimit, stopStrategy3_url, Remain_url
...
#  网格部署(含密度)
def gridbegin(griddata, userUuid, strategyId):
...
def gridStrategy(griddata):
    try:
        ...
        if strategyType == 1:
            ...
            print("网格{}密度单{}".format(strategyId, grid_orders))
                    ...
                    try:
                        ...
                        cancelres = requests.post(Cancel_url,
                                                  data={"direction": 1, "symbol": symbol, "platform": platform,
                                                        ...
                                                        "icebergId": strategyId})
                        cancelinfo = "用户{}策略{}平台{}撤销买单，返回结果{}".format(userUuid, strategyId, platform, cancelres.text)
                        # logger.info(cancelinfo)
                        print(cancelinfo)
                        deletebuysql = "delete from t_gridtrade where strategyId=%s and buyorderid=%s"
                        cur.execute(deletebuysql, (strategyId, grid_orders["buyorderid"]))
                        ...
                        if sellcode != 1:
                            sellerrorinfo = "您的{}平台{}资金不足或者交易所接口不通，导致量化策略下卖单失败，可在调整资金后再重新创建策略".format(platform,
                                                                                                      counterCoinName)
                            print(sellerrorinfo)
                            # logger.info(sellerrorinfo)
                            # 资产不足信息储存到redis
                            remainerrordata = {"strategyId": strategyId, "userUuid": userUuid,
                                               "apiAccountId": apiAccountId, "platform": platform,
                            ...
                            netprofit = round(profit - entryPrice * count * makerFee - newsellprice * count * makerFee,
                                              8)
                            sellinsertsql = "insert into t_gridtrade (userUuid,apiAccountId,strategyId,platform," \
                                            "symbol,sellprice,sellcount,sellorderid,sellstatus,sellordertime,profit," \
                                            "netprofit,uniqueId) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s); "
                            cur.execute(sellinsertsql, (
                                userUuid, apiAccountId, strategyId, platform, symbol, newsellprice, count,
                                newsellorderid, 0, newsellordertime, profit, netprofit, 1))
                            # 4、新挂一个买单
                            ...
                            buycode = dict1["code"]
                            if buycode != 1:
                                buyerrorinfo = "您的{}平台{}资金不足或者交易所接口不通，导致网格策略下买单失败，请停止当前策略，可在调整资金后再重新创建策略".format(
                                    platform,
                                    valueCoinName)
                                print(buyerrorinfo)
                    ...
                    try:
                        ...
                        if sellcode != 1:
                            ...
                            remainerrordata = {"strategyId": strategyId, "apiAccountId": apiAccountId,
                                               "userUuid": userUuid,
                                               "platform": platform, "coin": counterCoinName,
                                               "marktime": int(time.time() * 1000)}
                            r2.hset("errormark", strategyId, json.dumps(remainerrordata))
                        elif sellcode == 1:
                            newsellorderid = dict2["response"]["orderid"]
                            profit = round(gap * count, 8)
                            netprofit = round(
                                profit - (newsellprice - gap) * count * makerFee - newsellprice * count * makerFee, 8)
                            ...
                            cur.execute(updatesql1, (
                                ...
                                netprofit,
                                strategyId, grid_orders["buyorderid"]))
                            ...
                            if buycode != 1:
                                ...
                                # logger.info(buyerrorinfo)
                                # 资产不足信息储存到redis
                                remainerrordata = {"strategyId": strategyId, "platform": platform,
                                                   "coin": valueCoinName,
                                ...
                                cur.execute(buyinsertsql, (
                                    ...
                                    newbuyorderid, 0, newbuyordertime, 1))
                                # 4、修改缓存
                                selldata0 = {"userUuid": userUuid, "apiAccountId": apiAccountId,
                                             "strategyId": strategyId, "platform": platform,
                                             "symbol": symbol, "count": count, "sellprice": newsellprice,
                                             "sellorderid": newsellorderid}
                                buydata0 = {"userUuid": userUuid, "apiAccountId": apiAccountId,
                                            "strategyId": strategyId, "platform": platform,
                                            "symbol": symbol, "count": count, "buyprice": newbuyprice,
                                            "buyorderid": newbuyorderid}
                ...
                # 二、查询卖单和买单，并根据成交情况重新部署
                    ...
                    if sellnum == count:
                        ...
                        try:
                            ...
                            # 2、撤销对应的买单
                            cancelbuyordrinfo = json.loads(r2.get("grid:buy:" + str(strategyId) + ":0"))
                            cancelres = requests.post(Cancel_url,
                                                      data={"direction": 1, "symbol": symbol, "platform": platform,
                            ...
                            if sellcode != 1:
                                ...
                                if len(selectres) != 0:
                                    updatesql2 = "update t_gridtrade set sellorderid=%s,sellstatus=%s,sellordertime=%s where strategyId=%s and sellprice=%s and sellstatus=2"
                                    cur.execute(updatesql2, (
                                        newsellorderid, 0, newsellordertime, strategyId, newsellprice))
                                elif len(selectres) == 0:
                                    profit = (newsellprice - entryPrice) * count
                                    if profit <= 0:
                                        profit = -1 * profit
                                    netprofit = profit - entryPrice * count * makerFee - newsellprice * count * makerFee
                                    if netprofit <= 0:
                                ...
                                if buycode != 1:
                                    ...
                                    # 资产不足信息储存到redis
                                    remainerrordata = {"userUuid": userUuid, "apiAccountId": apiAccountId,
                                    ...
                                    cur.execute(buyinsertsql, (
                                        userUuid, apiAccountId, strategyId, platform, symbol, newbuyprice, count,
                                        newbuyorderid, 0, newbuyordertime,
                                        1))
                                    # 5、修改内存中订单信息
                                    selldata1 = {"userUuid": userUuid, "apiAccountId": apiAccountId,
                                                 "strategyId": strategyId, "platform": platform,
                                                 "symbol": symbol, "count": count, "sellprice": newsellprice,
                                                 "sellorderid": newsellorderid}
                                    buydata1 = {"userUuid": userUuid, "apiAccountId": apiAccountId,
                                                "strategyId": strategyId, "platform": platform,
                                                "symbol": symbol, "count": count, "buyprice": newbuyprice,
                                                "buyorderid": newbuyorderid}
                                    key_sell0 = "grid:sell:" + str(strategyId) + ":0"
                                    ...
                                    key_sell1 = "grid:sell:" + str(strategyId) + ":1"
                                    key_buy1 = "grid:buy:" + str(strategyId) + ":1"
                                    selldata0 = r2.get(key_sell1)
                                    buydata0 = r2.get(key_buy1)
                                    r2.set(key_sell0, selldata0)
                                    r2.set(key_buy0, buydata0)
                                    r2.set(key_sell1, json.dumps(selldata1))
                                    r2.set(key_buy1, json.dumps(buydata1))
                                    print("修改redis缓存中的订单")
                        except Exception as e:
                    ...
                    if buynum == count:
                        ...
                        try:
                            ...
                            buytradetime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                            # 1、撤销卖单
                            cancelsellorderinfo = json.loads(r2.get("grid:sell:" + str(strategyId) + ":1"))
                            cancelres1 = requests.post(Cancel_url,
                                                       data={"direction": 2, "symbol": symbol, "platform": platform,
                                                             "orderId": cancelsellorderinfo["sellorderid"],
                                                             "apiAccountId": apiAccountId,
                                                             "userUuid": userUuid, "source": 4,
                                                             "strategyId": strategyId, "icebergId": strategyId})
                            ...
                            sellcode = dict2["code"]  # 卖单状态
                            if sellcode != 1:
                                sellerrorinfo = "您的{}平台{}资金不足或者交易所接口不通，导致网格策略下卖单失败，请停止当前策略，可在调整资金后再重新创建开启".format(
                                    platform,
                                    counterCoinName)
                                print(sellerrorinfo)
                                # r.lpush("gridinfo:{}".format(userId), sellerrorinfo)  # 信息存储到redis
                                # 资产不足信息储存到redis
                                remainerrordata = {"strategyId": strategyId, "platform": platform,
                                                   "coin": counterCoinName,
                                                   "userUuid": userUuid, "apiAccountId": apiAccountId,
                                                   "marktime": int(time.time() * 1000)}
                                r2.hset("errormark", strategyId, json.dumps(remainerrordata))
                            elif sellcode == 1:
                                newsellorderid = dict2["response"]["orderid"]
                                ...
                                if buycode != 1:
                                    ...
                                    selldata0 = {"userUuid": userUuid, "apiAccountId": apiAccountId,
                                                 "strategyId": strategyId, "platform": platform,
                                                 "symbol": symbol,
                                                 "count": count, "sellprice": newsellprice,
                                                 "sellorderid": newsellorderid}
                                    buydata0 = {"userUuid": userUuid, "apiAccountId": apiAccountId,
                                                "strategyId": strategyId, "platform": platform,
                                                "symbol": symbol,
                    ...
                    try:
                        ...
                        if sellcode != 1:
                            ...
                            if len(selectres) == 1:
                                ...
                                #     newsellordertime, profit, netprofit, 1))
                                # 这种情形与卖单对应的是入场价的买单
                                sellinsertsql = "insert into t_gridtrade (userUuid,apiAccountId,strategyId,platform,symbol,buyprice,buycount,buystatus,buyordertime,buytradetime,sellprice,sellcount,sellorderid,sellstatus,sellordertime,profit,netprofit,uniqueId) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);"
                                cur.execute(sellinsertsql, (
        ...
        # 量化策略3（组合策略）逻辑
            ...
            counterCoinName = symbol.split("_")[0]  # 交易币
            valueCoinName = symbol.split("_")[1]  # 计价币
            initialTradeCoin0 = griddata["initialTradeCoin0"]  # 用户用于网格策略的交易币数量
            initialValCoin0 = griddata["initialValCoin0"]  # 计价币数量
            entryPrice0 = griddata["entryPrice0"]
            gap = griddata["gridSpacing"]  # 网格间距
            makerFee = griddata["makerFee"]  # 挂单手续费
            count = griddata["minTradeQuantity"]  # 每次下单量
            highprice = griddata["profitCeiling"]  # 止盈位
            lowprice = griddata["stopLossPrice"]  # 支撑位，止损
            finishType = griddata["finishType"]
            # currentprice = \
            #     [i["currentprice"] for i in currentpricelist if i["platform"] == platform and i["symbol"] == symbol][0]
            info = "量化策略{}正在执行中...".format(strategyId)
            print(info)
            ...
            if grid_orders != {}:
                ...
                if finishType == 1:  # 出场方式finishType为1的话，止盈止损出场
                    if currentprice != 0 and currentprice <= lowprice:
                        res = requests.post(stopStrategy3_url,
                                            data={"userUuid": userUuid, "apiAccountId": apiAccountId,
                                                  "strategyId": strategyId, "status": 6, "stopType": 2})
                        # logger.info("策略{}止损出场{}".format(strategyId, res.text))
                    elif currentprice >= highprice:
                        res = requests.post(stopStrategy3_url,
                                            data={"userUuid": userUuid, "apiAccountId": apiAccountId,
                                                  "strategyId": strategyId, "status": 2, "stopType": 2})
                        # logger.info("策略{}止盈出场{}".format(strategyId, res.text))
                    # 二、查询卖单和买单，并根据成交情况重新部署
                    else:
                        # 获取网格交易卖单和买单列表
                        sellnum = 0
                        ...
                        if sellnum == count:
                            ...
                            try:
                                ...
                                newsellprice = round((finishsellprice + gap), pricelimit[symbol][platform])
                                newsellordertime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                                newsellparams = {"direction": 2, "amount": count, "symbol": symbol,
                                                 "platform": platform, "price": newsellprice,
                                                 "apiAccountId": apiAccountId, "userUuid": userUuid, "source": 4,
                                                 "strategyId": strategyId, "icebergId": strategyId}
                                res2 = requests.post(Trade_url, data=newsellparams)
                                dict2 = json.loads(res2.content.decode())
                                ...
                                if sellcode != 1:
                                    ...
                                    if len(selectres) != 0:
                                        ...
                                        if profit <= 0:
                                            profit = -1 * profit
                                        netprofit = round(
                                            profit - entryPrice * count * makerFee - newsellprice * count * \
                                            makerFee, 8)
                                        if netprofit <= 0:
                                            netprofit = -1 * netprofit
                                        sellinsertsql = "insert into t_gridtrade (userUuid,apiAccountId,strategyId,platform,symbol,sellprice,sellcount,sellorderid,sellstatus,sellordertime,profit,netprofit,uniqueId) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);"
                                    ...
                                    newbuyprice = round((finishsellprice - gap), pricelimit[symbol][platform])
                                    newbuyordertime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                                    newbuyparams = {"direction": 1, "amount": count, "symbol": symbol,
                                                    "platform": platform, "price": newbuyprice,
                                                    "apiAccountId": apiAccountId, "userUuid": userUuid, "source": 4,
                                                    "strategyId": strategyId, "icebergId": strategyId}
                                    res1 = requests.post(Trade_url, data=newbuyparams)
                                    dict1 = json.loads(res1.content.decode())
                            ...
                            try:
                                ...
                                if sellcode != 1:
                                    ...
                                    netprofit = round(profit - (
                                            newsellprice - gap) * count * makerFee - newsellprice * count * \
                                                      makerFee, 8)
                                    updatesql1 = "update t_gridtrade set buystatus=%s,buytradetime=%s,sellprice=%s,sellcount=%s,sellorderid=%s,sellstatus=%s,sellordertime=%s,profit=%s,netprofit=%s where strategyId=%s and buyorderid=%s "
                                    cur.execute(updatesql1, (
                                        1, buytradetime, newsellprice, count, newsellorderid, 0, newsellordertime,
                                        profit, netprofit,
                                        strategyId, grid_buy_order["buyorderid"]))
                                    # 3、新挂一个买单（比之前的买单小一个网格）
                                    newbuyprice = round((finishbuyprice - gap), pricelimit[symbol][platform])
                                    ...
                                    if buycode != 1:
                                        ...
                                        buydata0 = {"userUuid": userUuid, "apiAccountId": apiAccountId,
                                                    "strategyId": strategyId, "platform": platform,
                                                    "symbol": symbol,
                                                    "count": count, "buyprice": newbuyprice,
                                                    "buyorderid": newbuyorderid}
                                        key_sell0 = "grid:sell:" + str(strategyId) + ":0"
                                        key_buy0 = "grid:buy:" + str(strategyId) + ":0"
                                        key_sell1 = "grid:sell:" + str(strategyId) + ":1"
                                ...
                                cur.close()
                                conn.close()
                if finishType == 2:  # 出场方式finishType为2的话，达到赢利点出场
                    expectedProfitRate = griddata["expectedProfitRate"]  # 期望收益
                    # 根据资产查询收益
                    ...
                    TradeCoin_amount = [i["remains"] for i in remaindict["response"] if i["coin"] == counterCoinName][0]
                    ValCoin_amount = [i["remains"] for i in remaindict["response"] if i["coin"] == valueCoinName][0]
                    profit = (TradeCoin_amount * currentprice + ValCoin_amount) - (
                            initialTradeCoin0 * entryPrice0 + initialValCoin0)
                    profitRate = profit / (initialTradeCoin0 * entryPrice0 + initialValCoin0)
                    print("策略{}当前收益{}，收益率{}".format(strategyId, profit, profitRate))
                    # 如果量化收益大于期望收益率+手续费率+溢价，出场
                    if profitRate >= expectedProfitRate + makerFee:
                        res = requests.post(stopStrategy3_url,
                                            data={"userUuid": userUuid, "apiAccountId": apiAccountId,
                                                  "strategyId": strategyId, "status": 2, "stopType": 2})
                        # logger.info(res.text)
                    else:
                        if currentprice != 0 and currentprice <= lowprice + gap:  # 暂停
                            pass
                        elif currentprice >= highprice - gap:  # 暂停
                            pass
                        # 二、查询卖单和买单，并根据成交情况重新部署
                        else:
                            # 获取网格交易卖单和买单列表
                            ...
                            try:
                                ...
                                sellinfo = "用户{}交易平台{},最小卖单订单号{},价格{},已成交量{}".format(userUuid, platform,
                                                                                     grid_sell_order["sellorderid"],
                                                                                     grid_sell_order["sellprice"],
                                                                                     sellnum)
                                print(sellinfo)
                            except Exception as e:
                                i = "用户{}网格策略{}交易平台{}查询订单{}出错{}，{}".format(userUuid, strategyId, platform,
                                                                           grid_sell_order["sellorderid"], e,
                                                                           sellquerydict)
                                print(i)
                                logger.error(i)
                            if sellnum != count:  # 如果卖单没有完全成交，则查询买单是否成交，否则不查询
                            ...
                            if sellnum == count:
                                ...
                                try:
                                    finishsellprice = grid_sell_order["sellprice"]  # 已经成交单的价格
                                    info2 = "量化策略{}已成交一个卖单，交易平台{}，成交价{}，成交数量{}，正在为您部署新的网格...".format(strategyId,
                                                                                                     platform,
                                                                                                     finishsellprice,
                                                                                                     sellnum)
                                    print(info2)
                                    ...
                                    # 2、撤销对应的买单
                                    cancelbuyordrinfo = json.loads(r2.get("grid:buy:" + str(strategyId) + ":0"))
                                    cancelres = requests.post(Cancel_url, data={"direction": 1, "symbol": symbol,
                                                                                "platform": platform,
                                                                                "orderId": cancelbuyordrinfo[
                                                                                    "buyorderid"],
                                                                                "apiAccountId": apiAccountId,
                                                                                "userUuid": userUuid, "source": 4,
                                                                                "strategyId": strategyId,
                                    ...
                                    sellcode = dict2["code"]  # 卖单状态
                                    if sellcode != 1:
                                        sellerrorinfo = "您的{}平台{}资金不足或者交易所接口不通，导致网格策略下卖单失败，可在调整资金后再重新创建开启".format(
                                            platform,
                                            counterCoinName)
                                        print(sellerrorinfo)
                                        # logger.info(sellerrorinfo)
                                        ...
                                        if len(selectres) != 0:
                                            ...
                                            sellinsertsql = "insert into t_gridtrade (userUuid,apiAccountId,strategyId,platform,symbol,sellprice,sellcount,sellorderid,sellstatus,sellordertime,profit,netprofit,uniqueId) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);"
                                            cur.execute(sellinsertsql, (
                                                userUuid, apiAccountId, strategyId, platform, symbol, newsellprice,
                                                count, newsellorderid, 0,
                                                newsellordertime, profit, netprofit, 1))
                                        # 4、新挂一个买单（比最新成交价小一个网格间距)
                                        newbuyprice = round((finishsellprice - gap), pricelimit[symbol][platform])
                                        newbuyordertime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                                        ...
                                        if buycode != 1:
                                            ...
                                            buyinsertsql = "insert into t_gridtrade (userUuid,apiAccountId,strategyId,platform,symbol,buyprice,buycount,buyorderid,buystatus,buyordertime,uniqueId) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);"
                                            cur.execute(buyinsertsql, (
                                                userUuid, apiAccountId, strategyId, platform, symbol, newbuyprice,
                                                count, newbuyorderid, 0,
                                                newbuyordertime,
                                                1))
                                            # 5、修改内存中订单信息
                                            selldata1 = {"userUuid": userUuid, "apiAccountId": apiAccountId,
                            ...
                            if buynum == count:
                                ...
                                try:
                                    finishbuyprice = grid_buy_order["buyprice"]
                                    info3 = "量化策略{}已成交一个买单，交易平台{}，成交价{}，成交数量{}，正在为您部署新的网格...".format(strategyId,
                                                                                                     platform,
                                                                                                     finishbuyprice,
                                                                                                     buynum)
                                    # r.lpush("gridinfo:{}".format(userId), info3)  # 信息存储到redis
                                    ...
                                    # 1、撤销卖单
                                    cancelsellorderinfo = json.loads(r2.get("grid:sell:" + str(strategyId) + ":1"))
                                    cancelres1 = requests.post(Cancel_url, data={"direction": 2, "symbol": symbol,
                                                                                 "platform": platform,
                                                                                 "orderId": cancelsellorderinfo[
                                                                                     "sellorderid"],
                                                                                 "apiAccountId": apiAccountId,
                                                                                 "userUuid": userUuid, "source": 4,
                                                                                 "strategyId": strategyId,
