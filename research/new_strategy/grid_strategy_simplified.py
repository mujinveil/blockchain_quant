...
from tools.Config import pricelimit, Queryorder_url, Cancel_url, Trade_url, amountlimit, premiumdict, Query_tradeprice_url
...
def clear_grid_remain(userUuid, apiAccountId, strategyId, platform, symbol, amount):
    try:
        ...
        conn = POOL_grid.connection()
        cur = conn.cursor()
        sellinsertsql = "insert into t_gridtrade (userUuid,apiAccountId,strategyId,platform,symbol,sellprice," \
                        "sellcount,sellorderid,sellstatus,sellordertime,uniqueId) values (%s,%s,%s,%s,%s,%s,%s,%s,%s," \
                        "%s,%s); "
...
def gridbegin(griddata, userUuid, strategyId):
    ...
    try:
        ...
        buyinsertsql = "insert into t_gridtrade (userUuid,apiAccountId,strategyId,platform,symbol,buyprice,buycount," \
                       "buyorderid,buystatus,buyordertime,uniqueId) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s); "
...
def gridbegin2(griddata, userUuid, strategyId):
    ...
    try:
        ...
        sellordertime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        sellinsertsql = "insert into t_gridtrade (userUuid,apiAccountId,strategyId,platform,symbol,sellprice," \
                        "sellcount,sellorderid,sellstatus,sellordertime,profit,netprofit,uniqueId) values (%s,%s,%s," \
                        "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s); "
        cur.execute(sellinsertsql,
                    (userUuid, apiAccountId, strategyId, platform, symbol, sellprice, count, sellorderid, 0,
                     sellordertime, profit, netprofit, 1))
...
def gridStrategy(griddata, currentpricelist):
                    ...
                    try:
                        ...
                        if sellcode != 1:
                            ...
                            if len(selectres) == 1:
                                ...
                                if profit <= 0:
                                    profit = -1 * profit
                                netprofit = profit - entryPrice * count * makerFee - newsellprice * count * \
                                            makerFee
                                if netprofit <= 0:
                                ...
                                cur.execute(sellinsertsql, (
                                    ...
                                    newsellprice, count, newsellorderid, 0, newsellordertime, profit, netprofit, 1))
                    ...
                    try:
                        ...
                        if sellcode != 1:
                            ...
                            netprofit = profit - (newsellprice - gap) * count * makerFee - newsellprice * count * \
                                        makerFee
                            updatesql1 = "update t_gridtrade set buystatus=%s,buytradetime=%s,sellprice=%s," \
                            ...
                            cur.execute(updatesql1, (
                                ...
                                netprofit, strategyId, grid_buy_order["buyorderid"]))
                            ...
                            if buycode != 1:
                                ...
                                newbuyorderid = dict1["response"]["orderid"]
                                buyinsertsql = "insert into t_gridtrade (userUuid,apiAccountId,strategyId,platform," \
                                               "symbol,buyprice,buycount,buyorderid,buystatus,buyordertime," \
                                               "uniqueId) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s); "
                                cur.execute(buyinsertsql, (
                                    userUuid, apiAccountId, strategyId, platform, symbol, newbuyprice, count,
                                    newbuyorderid, 0, newbuyordertime, 1))
                                # 4、修改内存中订单信息
        ...
        try:
            ...
            if not strategyIdlist:
                ...
                for griddata in gridstrategydatalist:
                    gridThreads.append(Thread(target=gridStrategy, args=(json.loads(griddata), currentpricelist)))
